...
const VoiceEngine = {
  ...
  config: {
    ...
    pitch: 0.95,
    volume: 1.0,
    vadDelay: 3000,   // 3 segundos de silêncio → processa
    lang: 'pt-BR'
  },
  ...
  init() {
    ...
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
      console.warn('⚠️ SpeechRecognition não suportado neste navegador.');
  ...
  async start() {
    ...
    recognition.onstart = () => {
      ...
      document.getElementById('vadStatus').textContent = '🎙️ Ouvindo...';
    };
    ...
    recognition.onresult = (event) => {
      ...
      if (interim) {
        document.getElementById('vadStatus').textContent = '💬 ' + interim.substring(0, 40) + '...';
      }

      // Quando tem resultado final → inicia timer VAD
      if (final.trim()) {
  ...
  onSpeechDetected(text) {
    ...
    document.getElementById('vadStatus').textContent = `⏳ Aguardando 3s...`;

    // Timer VAD — 3 segundos de silêncio
    this.state.silenceTimer = setTimeout(() => {
      document.getElementById('vadStatus').textContent = '🧠 Processando...';
      this.stop();
  ...
  pause() {
    ...
    if (!this.state.isPaused) {
      ...
      document.getElementById('vadStatus').textContent = '⏸️ Pausado';
  ...
  speak(text, onEnd) {
    ...
    // Aplica voz PT-BR se disponível
    ...
    utterance.onend = () => {
      ...
      if (onEnd) onEnd();
      // Após falar, reinicia escuta automaticamente
      setTimeout(() => {
        if (!this.state.isListening) {
          this.start();
        }
