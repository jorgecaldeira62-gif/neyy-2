// ============================================================
// VOXIS — drive.js
// Google Drive Backup — preparado para ativar com OAuth
// ============================================================

const DriveSync = {

    // 🔧 CONFIGURE AQUI seu Client ID do Google Cloud Console
    config: {
        clientId:  'SEU_GOOGLE_CLIENT_ID_AQUI',
        apiKey:    'SUA_GOOGLE_API_KEY_AQUI',
        scope:     'https://www.googleapis.com/auth/drive.file',
        folderId:  null,   // Será criado automaticamente
    ...
    init() {
        if (
            ...
            console.log('💾 Drive: aguardando configuração.');
            document.getElementById('driveStatus').textContent = '⚙️ Config';
        }
    },
    ...
    initGIS() {
        this.tokenClient = google.accounts.oauth2.initTokenClient({
            ...
            callback:  (response) => {
                ...
                document.getElementById('driveStatus').textContent = '✅ Ativo';
                document.getElementById('driveStatus').className   = 'badge badge-green';
                console.log('💾 Drive autorizado!');
            }
        });
    },
    ...
    authorize() {
        if (!this.isConfigured) {
            alert('⚙️ Configure o Google Drive em js/drive.js\n\nAcesse: https://console.cloud.google.com');
            return;
        }
        if (this.tokenClient) {
    ...
    async backup() {
        ...
        try {
            ...
            App.addMessage('voxis', `💾 Backup salvo no Google Drive!\nArquivo: ${filename}`);
            App.setStatus('', 'Pronto');

        } catch(e) {
            console.error('Erro no backup:', e);
            App.addMessage('voxis', '❌ Erro ao fazer backup no Drive. Tente novamente.');
            App.setStatus('', 'Pronto');
        }
    },
    ...
    async restore() {
        ...
        try {
            ...
            if (files.length === 0) {
                App.addMessage('voxis', '💾 Nenhum backup encontrado no Drive.');
                return;
            }
            ...
            App.addMessage('voxis', `✅ Restaurado backup de ${new Date(backup.timestamp).toLocaleString('pt-BR')}!`);

        } catch(e) {
            console.error('Erro ao restaurar:', e);
            App.addMessage('voxis', '❌ Erro ao restaurar do Drive.');
        }
    }
};
