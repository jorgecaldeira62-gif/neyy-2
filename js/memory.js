// ============================================================
// VOXIS — memory.js
// Memória: histórico, estatísticas, export/import
// ============================================================
...
const Memory = {
  ...
  init() {
    ...
    this.setData(data);
    console.log('💾 Memory iniciado. Sessão:', data.sessions);
  },
  ...
  exportHistory() {
    ...
    if (!data.history || data.history.length === 0) {
      alert('⚠️ Nenhum histórico para exportar ainda.');
      return;
    }

    // Formato TXT legível
    let txt = `VOXIS — Histórico de Conversa\n`;
    txt += `Exportado em: ${new Date().toLocaleString('pt-BR')}\n`;
    txt += `Total de interações: ${data.interactions}\n`;
    txt += `Sessões: ${data.sessions}\n`;
    txt += `${'='.repeat(50)}\n\n`;

    data.history.forEach(msg => {
      const time = new Date(msg.time).toLocaleString('pt-BR');
      const who = msg.role === 'user' ? '👤 VOCÊ' : '◈ VOXIS';
      txt += `[${time}] ${who}:\n${msg.text}\n\n`;
    });
  ...
  importHistory(jsonText) {
    try {
      ...
      if (!imported.history) throw new Error('Formato inválido');

      const data = this.getData();
      // Mescla históricos sem duplicar
      ...
      this.setData(data);
      App.addMessage('voxis', `✅ Importei ${newMsgs.length} mensagens do histórico!`);
    } catch (e) {
      App.addMessage('voxis', '❌ Não consegui ler esse arquivo. Certifique-se que é um JSON do VOXIS.');
    }
  },

  // ===== LIMPAR HISTÓRICO =====
  clearHistory() {
    if (!confirm('⚠️ Tem certeza que quer limpar o histórico de conversa?\n(O aprendizado será mantido)')) return;
    ...
    container.innerHTML = `
            <div class="chat-welcome">
                ...
                <h2>Olá! Eu sou o <strong>VOXIS</strong></h2>
                <p>Histórico limpo. Pronto para uma nova conversa!</p>
            </div>
        `;
  },
